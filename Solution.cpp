...
void LEETCODE::rotate(vector<int>& nums, int k) {
  /* rotation counted less than 1 complete cycle */
  k = k % nums.size();
  
  /* store first nums.size() - k elements from nums */
  vector<int> temp {nums.begin(), nums.end()-k};
  /* erase first nums.size()-k elements*/
  nums.erase(nums.begin(), nums.end()-k);
  /* append first nums.size()-k to the last k elements of nums */
